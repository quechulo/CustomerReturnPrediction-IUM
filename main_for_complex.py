if __name__ == '__main__':
    ...
    for i in range(30, 36):
        weeks = i
        prediction = predict(weeks, models)
        pr.append(prediction)
        diff = round(abs(data_normal[i] - prediction), 1)
        differences.append(diff)

    print(np.mean(differences))


    plt.plot(data_normal, label="observed")
    plt.plot(pr, label="predicted")
    x = [i for i in range(0, len(data_normal))]
    y = data_normal
    plt.plot(x, y)
    my_xticks = [None for i in range(0, len(data_normal))]
